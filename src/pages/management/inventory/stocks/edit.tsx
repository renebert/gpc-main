...
const Edit: FC<IProps> = ({ data, caller }) => {
	...
	const handleSubmit = async (data: Stock) => {
		nc.processing.show();
		let res = await req.post(`${g.API_URL}/inventory/stock/save`, data);
		if (res.success) {
	...
	const submitForm = () => {
		(
			ps.Get("create-stock-form-setExecSubmit")?.dispatch as React.Dispatch<
